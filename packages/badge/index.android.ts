...
export function applyNotification(notification: Notification, badge: number, channelId?: string) {
	...
	const builder = new NBuilder(Utils.ad.getApplicationContext()).setContentTitle(notification.title).setContentText(notification.body).setSmallIcon(Utils.ad.getApplicationContext().getApplicationInfo().icon);
